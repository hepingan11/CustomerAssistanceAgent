...
function findRelativeCandidate(messageEl, patterns) {
  const candidates = Array.from(messageEl.querySelectorAll("*")).filter((el) => {
    const value = `${el.tagName} ${el.id || ""} ${el.className || ""} ${el.getAttribute("datetime") || ""}`.toLowerCase();
    return patterns.some((pattern) => pattern.test(value)) && normalizeText(el.textContent);
  });
  return candidates.sort((a, b) => normalizeText(a.textContent).length - normalizeText(b.textContent).length)[0] || null;
}
...
function autoDetectSelectors(sampleText) {
  ...
  const senderEl = findRelativeCandidate(messageEl, [/author/, /sender/, /user/, /name/, /nick/, /from/, new RegExp(zh.agent), new RegExp(zh.customer)]);
  const timeEl = findRelativeCandidate(messageEl, [/time/, /date/, /created/, /datetime/]);
  ...
  const senderSelector = senderEl ? selectorFor(senderEl, messageEl) : "";
  const timeSelector = timeEl ? selectorFor(timeEl, messageEl) : "";
  ...
  return {
    ...
    preview: {
      ...
      sender: senderEl ? normalizeText(senderEl.textContent).slice(0, 60) : "",
      time: timeEl ? normalizeText(timeEl.textContent).slice(0, 60) : ""
    }
  };
}
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  if (message.type === "CAA_GET_STATE") {
    chrome.storage.local.get(["captureStatus", "lastSuggestion", LOG_KEY], (result) => {
      sendResponse({ status: result.captureStatus || zh.notEnabled, suggestion: result.lastSuggestion || lastSuggestion, logs: result[LOG_KEY] || [] });
    });
    return true;
  }
  if (message.type === "CAA_AUTO_DETECT") {
    try {
      const result = autoDetectSelectors(message.sampleText || "");
